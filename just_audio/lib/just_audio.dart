...
class AudioPlayer {
  ...
  StreamSubscription<VisualizerWaveformCaptureMessage>? _visualizerWaveformSubscription;
  ...
  AudioPlayer({
    ...
    playbackEventStream.pairwise().listen((pair) {
      ...
      if (curr.currentIndex != prev.currentIndex) {
        ...
        _positionDiscontinuitySubject
            .add(PositionDiscontinuity(PositionDiscontinuityReason.autoAdvance, prev, curr));
      } else {
    ...
    _androidAudioSessionIdSubject.addStream(playbackEventStream
        ...
        .distinct()
        .handleError((Object err, StackTrace stackTrace) {/* noop */}));
    _sequenceStateSubject.addStream(Rx.combineLatest5<List<IndexedAudioSource>?, List<int>?, int?,
        bool, LoopMode, SequenceState?>(
      sequenceStream,
    ...
    _playerStateSubject.addStream(Rx.combineLatest2<bool, PlaybackEvent, PlayerState>(playingStream,
            playbackEventStream, (playing, event) => PlayerState(playing, event.processingState))
        .distinct()
        .handleError((Object err, StackTrace stackTrace) {/* noop */}));
  ...
  Future<Duration?> setAudioSource(
    ...
    _playbackEventSubject.add(_playbackEvent = PlaybackEvent(
        currentIndex: initialIndex ?? 0, updatePosition: initialPosition ?? Duration.zero));
    _audioSource = source;
    _broadcastSequence();
    Duration? duration;
  ...
  Future<void> shuffle() async {
    ...
    _updateShuffleIndices();
    await (await _platform)
        .setShuffleOrder(SetShuffleOrderRequest(audioSourceMessage: _audioSource!._toMessage()));
  }
      ...
      default:
        try {
          ...
          _positionDiscontinuitySubject.add(PositionDiscontinuity(
              PositionDiscontinuityReason.seek, prevPlaybackEvent, _playbackEvent));
  ...
  Future<Duration?>? _setPlatformActive(bool active,
    ...
    void subscribeToEvents(AudioPlayerPlatform platform) {
      ...
      _visualizerWaveformSubscription = platform.visualizerWaveformStream.listen((message) {
      ...
      if (audioSource != null) {
        try {
          final initialSeekValues =
              _initialSeekValues ?? _InitialSeekValues(position: position, index: currentIndex);
          _initialSeekValues = null;
          final duration =
              await _load(platform, _audioSource!, initialSeekValues: initialSeekValues);
          if (checkInterruption()) return platform;
...
class SequenceState {
  ...
  SequenceState(this.sequence, this.currentIndex, this.shuffleIndices, this.shuffleModeEnabled,
      this.loopMode);
